...
mkRunAutomation port broadcastChan = do
  ...
  devicesRaw <- view devicesRawJSON
  groupsRaw <- view groupsRawJSON
  ...
  where
    ...
    ws logger' devicesRaw groupsRaw devicesTV groupsTV daemonBC pending  = do
      ...
      WS.sendTextData conn =<< readTVarIO devicesRaw

      logDebugMsg logger' "Sending Group data to client"
      WS.sendTextData conn =<< readTVarIO groupsRaw

      broadcastChanCopy <- atomically . dupTChan $ broadcastChan

      devices' <- readTVarIO devicesTV
      groups' <- readTVarIO groupsTV

      let
        cleanupMessages = logDebugMsg logger' $
          "HTTP cleanup messages - " <> (T.pack . show $ devices') <> " - " <> (T.pack . show $ groups')

      flip finally cleanupMessages $
        WS.withPingThread conn 30 (pure ()) $ concurrently_
          ...
          (forever $ do
              ...
              logDebugMsg logger' $
                "Received via WebSocket: " <> (T.pack $ show received)
          ...
          (forever $ do
              ...
              logDebugMsg logger' $
                "Received on broadcastChan: " <> (T.pack $ show msg)
              case msg of
